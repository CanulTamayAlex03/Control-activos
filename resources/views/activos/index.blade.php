...
<div class="container-fluid px-0">
    <div class="row align-items-center mb-2 mx-0">
        <div class="col-md-8 px-0">
            <div class="d-flex gap-2">
                ...
                @if(!isset($viewMode) || $viewMode == 'card')
                <a href="{{ route('activos.index', array_merge(request()->all(), ['view_mode' => 'table'])) }}"
                ...
                @else
                <a href="{{ route('activos.index', array_merge(request()->all(), ['view_mode' => 'card'])) }}"
                    class="btn btn-sm btn-secondary d-inline-flex align-items-center justify-content-center"
                    style="width: 32px; height: 32px; padding: 0;"
                    title="Ver en tarjeta">
                    <i class="fas fa-id-card"></i>
                </a>
                @endif
            </div>

            @if(request('search') && (!$activo || !$activo->folio) && (!isset($viewMode) || $viewMode == 'card'))
        ...
        <div class="col-md-4 text-end px-0 d-flex justify-content-end align-items-center">
            @can ('editar activos')
            @if($activo && (!isset($viewMode) || $viewMode == 'card'))
                    ...
                    <div class="section-card w-100">
                        ...
                        <div class="section-body">
                            ...
                            <div class="info-row">
                                ...
                                @php
                                $esMayorIgualBaseCosto = $activo->base_costo && $activo->costo >= $activo->base_costo;
                                @endphp
                            
                                <span class="info-value fw-bold {{ $esMayorIgualBaseCosto ? 'text-success' : 'text-danger' }}">
                                    ${{ number_format($activo->costo, 2) }}
                                </span>
                            
                                @if($activo->base_costo)
                                <div class="small text-muted">
                                    Costo base: ${{ number_format($activo->base_costo, 2) }}
                                </div>
                                @endif
                            </div>
                            <div class="info-row">
    ...
    @else
    @include('activos.partials.tabla-activos')
    @endif
</div>
